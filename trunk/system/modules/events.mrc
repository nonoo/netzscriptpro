...
on ^1:PART:*: {
  ...
  if (($nick($chan,0) = 2) && ($me !isop $chan)) { /echo $color(info) -st *** OpHopot hajtottam végre a $chan csatin! | .timer 1 0 /hop $chan 1 | /halt }
  if ($1) {
  ...
  if ($nick != $me) {
    ...
    while (%i > 0) {
      if ($nick ison $chan(%i)) {
        ...
        ; ophop
        if ($chan(%i) != &servers) && ($nick($chan(%i),0) = 2) && ($me !isop $chan(%i)) { /echo $color(info) -st *** OpHopot hajtottam végre a $chan(%i) csatin! | /hop $chan(%i) 1 }
      }
      dec %i 1
    }
