...
ctcp ^1:PAGE: {
  ...
  if (%pager) || ( (%away_pager) && ($away) ) {
    ...
    if (%tooltip_pagenel) && (!$appactive) {
      var %a $tip(pager, Pager: $nick, $nick paget k¸ldˆtt!, $null, system\img\warning.ico, $null, /q $nick )
    }
  }
  halt
}
...
on 1:FILERCVD:*.*: {
  ...
  if (.jpg isin $nopath($filename)) || (.jpeg isin $nopath($filename)) || (.gif isin $nopath($filename)) || (.png isin $nopath($filename)) || (.bmp isin $nopath($filename)) || (.mpg isin $nopath($filename)) || (.mpeg isin $nopath($filename)) || (.avi isin $nopath($filename)) || (.wmv isin $nopath($filename)) || (.mov isin $nopath($filename)) || (.mp3 isin $nopath($filename)) || (.wav isin $nopath($filename)) || (.ogg isin $nopath($filename)) || (.txt isin $nopath($filename)) || (.pdf isin $nopath($filename)) || (.exe isin $nopath($filename)) || (.mpe isin $nopath($filename)) {
    ...
    if (!$appactive) {
      var %a $tip(dcc, DCC: $nick, Megjˆtt $laz($nopath($filename)) $nopath($filename), $null, system\img\dcc.ico, $null, .timer 1 0 /run $filename )
    }
  }
  else {
    if (!$appactive) && (%tooltip_dccnel) {
      var %a $tip(dcc, DCC: $nick, Megjˆtt $laz($nopath($filename)) $nopath($filename), $null, system\img\dcc.ico )
    }
  }
  halt
}
on 1:FILESENT:*: {
  ...
  if (!$appactive) && (%tooltip_dccnel) {
    var %a $tip(dcc, DCC: $nick, ¡tment $laz($nopath($filename)) $nopath($filename), $null, system\img\dcc.ico )
  }
  halt
}
on 1:SENDFAIL:*: {
  ...
  if (!$appactive) && (%tooltip_dccnel) {
    var %a $tip(dcc, DCC hiba: $nick, Nem siker¸lt elk¸ldeni $+  $+ $color(nick) $nick $+ $toldalek($nick,-nak,-nek) $laz($nopath($filename)) $nopath($filename) f·jlt!, $null, system\img\warning.ico )
  }
  halt
}
on 1:GETFAIL:*: {
  ...
  if (!$appactive) && (%tooltip_dccnel) {
    var %a $tip(dcc, DCC hiba: $nick, Nem siker¸lt fogadni $laz($nopath($filename)) $nopath($filename) f·jlt $nick $+ $toldalek($nick,-tÛl,tıl) $+ !, $null, system\img\warning.ico )
  }
  halt
}
;END
