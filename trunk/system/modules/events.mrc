...
on 1:FILESENT:*: {
  ...
  if (!$appactive) && (%tooltip_dccnel) {
    var %tip $tip(dccsent_ $+ $nick, DCC: $nick, ¡tment $laz($nopath($filename)) $nopath($filename), $null, system\img\dcc.ico )
  }
  halt
}
on 1:SENDFAIL:*: {
  ...
  if (!$appactive) && (%tooltip_dccnel) {
    var %tip $tip(dccsendfail_ $+ $nick, DCC hiba: $nick, Nem siker¸lt elk¸ldeni $+  $+ $color(nick) $nick $+  $+ $toldalek($nick,-nak,-nek) $laz($nopath($filename)) $nopath($filename) f·jlt!, $null, system\img\warning.ico )
  }
  halt
}
on 1:GETFAIL:*: {
