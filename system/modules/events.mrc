  ...
  if ($2 == SEND) {
    if (!$appactive) {
      ...
      if (%tooltip_dccnel) && (!$appactive) {
        var %tip $tip(dccget_ $+ $nick, DCC GET: $nick, $nick $laz($3) $+  $3 f·jlt szeretnÈ elk¸ldeni neked, $null, system\img\query.ico, $null, /dcc get $nick $3)
      }
    }
...
ctcp ^1:PAGE: {
  ...
  if (%pager) || ( (%away_pager) && ($away) ) {
    ...
    if (%tooltip_pagenel) && (!$appactive) {
      var %tip $tip(pager_ $+ $nick, Pager: $nick, $nick paget k¸ldˆtt!, $null, system\img\warning.ico, $null, /q $nick )
    }
  }
  halt
}
...
on ^1:ACTION:*:#:{
  ...
  /set -n %tempszoveg $replace($1-,√ú,‹,√ü,¸,√≥,Û,≈,ı,√∫,˙,√ä,È,√•,·,≈π,˚,√≠,Ì,√°,·,√ñ,÷,√ì,”,√â,…,√Å,¡,ı∞,€,√ç,Õ,√©,È,ı±,˚,√º,¸,√∂,ˆ,√ö,⁄,ıë,ı,ıê,’,√É¬º,˚)
  ; highlight
...
on ^1:TEXT:*:#:{
  ...
  ; webchat unicode karakterek
  /set -n %tempszoveg $replace($1-,√ú,‹,√ü,¸,√≥,Û,≈,ı,√∫,˙,√ä,È,√•,·,≈π,˚,√≠,Ì,√°,·,√ñ,÷,√ì,”,√â,…,√Å,¡,ı∞,€,√ç,Õ,√©,È,ı±,˚,√º,¸,√∂,ˆ,√ö,⁄,ıë,ı,ıê,’)
  ; plusz jelek
  var %plusz
...
on 1:FILERCVD:*.*: {
  ...
  if (.jpg isin $nopath($filename)) || (.jpeg isin $nopath($filename)) || (.gif isin $nopath($filename)) || (.png isin $nopath($filename)) || (.bmp isin $nopath($filename)) || (.mpg isin $nopath($filename)) || (.mpeg isin $nopath($filename)) || (.avi isin $nopath($filename)) || (.wmv isin $nopath($filename)) || (.mov isin $nopath($filename)) || (.mp3 isin $nopath($filename)) || (.wav isin $nopath($filename)) || (.ogg isin $nopath($filename)) || (.txt isin $nopath($filename)) || (.pdf isin $nopath($filename)) || (.exe isin $nopath($filename)) || (.mpe isin $nopath($filename)) {
    ...
    if (!$appactive) {
      var %a $tip(dccrcvd_ $+ $nick, DCC: $nick, Megjˆtt $laz($nopath($filename)) $nopath($filename), $null, system\img\dcc.ico, $null, .timer 1 0 /run $filename )
    }
  }
  else {
    if (!$appactive) && (%tooltip_dccnel) {
      var %a $tip(dccrcvd_ $+ $nick, DCC: $nick, Megjˆtt $laz($nopath($filename)) $nopath($filename), $null, system\img\dcc.ico )
    }
  }
  halt
}
on 1:FILESENT:*: {
  /echo $color(info) -atg *** ¡tment $laz($nopath($filename)) $nopath($filename) $+  $+ $color(nick) $nick $+  $+ $color(info) $+ $toldalek($nick,-nak,-nek) $+ !
  if (!$appactive) && (%tooltip_dccnel) {
    var %tip $tip(dccsent_ $+ $nick, DCC SEND: $nick, ¡tment $laz($nopath($filename))  $+ $nopath($filename), $null, system\img\dcc.ico )
  }
  halt
}
on 1:SENDFAIL:*: {
  echo $color(info2) -atg *** Nem siker¸lt elk¸ldeni $+  $+ $color(nick) $nick $+  $+ $color(info2) $+ $toldalek($nick,-nak,-nek) $laz($nopath($filename)) $nopath($filename) f·jlt!
  if (!$appactive) && (%tooltip_dccnel) {
    var %tip $tip(dccsendfail_ $+ $nick, DCC hiba: $nick, Nem siker¸lt elk¸ldeni $nick $+ $toldalek($nick,-nak,-nek) $laz($nopath($filename))  $+ $nopath($filename) f·jlt!, $null, system\img\warning.ico )
  }
  halt
}
on 1:GETFAIL:*: {
  echo $color(info2) -atg *** Nem siker¸lt fogadni $laz($nopath($filename)) $nopath($filename) f·jlt $+  $+ $color(nick) $nick $+  $+ $color(info2) $+ $toldalek($nick,-tÛl,tıl) $+ !
  if (!$appactive) && (%tooltip_dccnel) {
    var %tip $tip(dccgetfail_ $+ nick, DCC hiba: $nick, Nem siker¸lt fogadni $laz($nopath($filename))  $+ $nopath($filename) f·jlt $nick $+ $toldalek($nick,-tÛl,tıl) $+ !, $null, system\img\warning.ico )
  }
  halt
}
;END
